...
public class LockPatternActivity extends Activity {
    ...
    public static final String EXTRA_MAX_RETRIES = CLASSNAME + ".max_retries";
    ...
    private RelativeLayout mLockPatternViewLayout;
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	if (BuildConfig.DEBUG) Log.d(CLASSNAME, "ClassName = " + CLASSNAME);
        ...
        if (getIntent().hasExtra(EXTRA_MAX_RETRIES)){
        	mMaxRetries = getIntent().getIntExtra(EXTRA_MAX_RETRIES, 3);
        }
       ...
       initContentView();
       mLockPatternViewLayout = (RelativeLayout)findViewById(R.id.LockPatternViewLayout);
       
       boolean shouldAddBackground = getIntent().getExtras().getBoolean("shouldAddBackground", false);
       
       int logoImage = getIntent().getExtras().getInt("logoImage", 0);
       
       
       if (shouldAddBackground){
           LinearLayout rootLayout = (LinearLayout) findViewById(R.id.LockPatternActivityRootLayout);
           rootLayout.setBackgroundColor(Color.argb(220, 231, 231, 231));
       }
       
       if(logoImage > 0) {
           ImageView view = (ImageView)findViewById(R.id.app_logo);

           findViewById(R.id.logoView).setVisibility(View.VISIBLE);
           findViewById(R.id.logoSpacer).setVisibility(View.GONE);;

           view.setImageDrawable(getResources().getDrawable(logoImage));
       }
    ...
    private void initContentView() {
        ...
        if (ACTION_CREATE_PATTERN.equals(getIntent().getAction())) {
            ...
            // Anton start
            if (mMaxRetries == 3){
            	mTextInfo.setText("draw pattern to confirm");
            }
            // Anton end
        ...
        new LoadingDialog<Void, Void, Boolean>(this, false) {
            ...
            protected Boolean doInBackground(Void... params) {
                if (ACTION_COMPARE_PATTERN.equals(getIntent().getAction())) {
                    char[] correctPattern = getIntent().getCharArrayExtra(EXTRA_PATTERN);
                    if (correctPattern == null)
                        correctPattern = Settings.Security.getPattern(LockPatternActivity.this);
                    if (correctPattern != null) {
                        if (mEncrypter != null){
                            return pattern.equals(mEncrypter.decrypt(LockPatternActivity.this, correctPattern));
                        }
                        else {
                        	char[] receivedPattern = LockPatternUtils.patternToSha1(pattern).toCharArray();
                            return Arrays.equals(correctPattern, receivedPattern);
                        }
                    }
                }// ACTION_COMPARE_PATTERN
                else if (ACTION_VERIFY_CAPTCHA.equals(getIntent().getAction())) {
    ...
    private final LockPatternView.OnPatternListener mLockPatternViewListener = new LockPatternView.OnPatternListener() {
        ...
        public void onPatternDetected(List<Cell> pattern) {
            if (ACTION_CREATE_PATTERN.equals(getIntent().getAction())) {
                ...
                if (!DisplayMode.Animate.equals(mLockPatternView.getDisplayMode())) doComparePattern(pattern);
            }// ACTION_VERIFY_CAPTCHA
        }// onPatternDetected()
